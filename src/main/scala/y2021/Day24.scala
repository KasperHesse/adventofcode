  ...
  def pairBundles(bundles: List[List[Instr]], div1: List[(List[Instr], Int)], paired: List[((List[Instr], Int), (List[Instr], Int))], cnt: Int = 0): List[((List[Instr], Int), (List[Instr], Int))] = {
    bundles match {
      case head::tail => head(4) match {
        ...
        case _ => throw new IllegalArgumentException(s"Instruction at (4) in bundles was not a y2021.DivRI. y2021.Instr=${head(4)}, bundle=$head")
      }
      case Nil => paired
    }
  }
...
object Day24 extends App {
  Solution.solve2021(new Day24, 24, 1, 1)
}