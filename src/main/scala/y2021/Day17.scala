...
class Day17 extends Solution {
  override def solvePart1(inp: List[String]): Any = {
    ...
    //When bullet falls down, it has velocity -y2021.Y at y=0 given that it start with velocity +y2021.Y
    //Next, it falls with velocity (-y2021.Y-1) and thus falls (-y2021.Y-1) steps
    //To just reach the bottom of the target zone, initial y-velocity should be (abs(yMin)-1)
    val Y = yMax.toInt.abs

    //Height reached is 1+2+3+...y2021.Y-1
    (Y-1)*Y/2
  }
...
object Day17 extends App {
  Solution.solve2021(new Day17, 17, 45, 112)
}
