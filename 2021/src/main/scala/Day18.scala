class Day18 extends Solution {
  ...
  private def doExplode(sn: SnailfishNum, depth: Int): (SnailfishNum, Int, Dir) = {
    ...
    sn match {
      ...
      case Pair(l, r) => if (depth == 3) {
        ...
        //If v >= 0, either propagate into r (if dir==Left()), or return back up (if dir==Right())
        (v, dir) match {
  ...
  override def solvePart1(inp: List[String]): Any = {
    ...
    //A number can either be a Lit, or it can be a Pair
...
object Day18 extends App {
  ...
  Solution.solve(new Day18, 18, 4140, 3993)
}